...
// Storage mantém alunos em localStorage e usa API para salas
const Storage = {
  _get(key) {
...
const UI = {
  ...
  async renderHistoricoList(idSala) {
    ...
    try {
      ...
      // fallback para possíveis dados antigos em localStorage
...
const App = {
  ...
  saveSalaFromState() {
    ...
    // Garantir que a lista de alunos venha dos checkboxes selecionados
    const tbody = document.getElementById('listaAlunos');
    if (tbody) {
      const selecionados = Array.from(tbody.querySelectorAll('.aluno-checkbox:checked'))
        .map(cb => cb.getAttribute('data-id'))
        .filter(Boolean);
      App.state.newSala.alunos = selecionados;
    }
    ...
    (async () => {
      const saved = await Storage.saveSala(salaToSave);
      if (!saved) return;
  ...
  actions: {
    ...
    async openChamada(idSala) {
      ...
      // para que funcione em qualquer navegador / estado de localStorage.
      const todayIso = Utils.formatDateIso();
      const chamadas = Storage.getChamadasBySala(idSala);
      let chamadaExistente = chamadas.find(c => (c.dataISO === todayIso));
    ...
    async showChamadaDetails(idSala, idChamada) {
      ...
      const chApi = chamadas.find(c => String(c.id) === String(idChamada));
      if (!chApi) {
